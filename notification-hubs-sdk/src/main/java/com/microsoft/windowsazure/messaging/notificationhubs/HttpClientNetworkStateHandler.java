...
class HttpClientNetworkStateHandler extends HttpClientDecorator implements NetworkStateHelper.Listener {
    ...
    public synchronized ServiceCall callAsync(String url, String method, Map<String, String> headers, CallTemplate callTemplate, ServiceCallback serviceCallback) {
        ...
        if (mNetworkStateHelper.isNetworkConnected()) {
            ...
            mCalls.add(call);
            Log.d("ANH", "Call triggered with no network connectivity, waiting network to become available...");
        }
        return call;
    }
    ...
    public synchronized void onNetworkStateUpdated(boolean connected) {
        if (connected && mCalls.size() > 0) {
            Log.d("ANH", "Network is available. " + mCalls.size() + " pending call(s) to submit now.");
